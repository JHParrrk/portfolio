...
import { useMutationCreateBoard } from '@/shared/hooks/mutations/useMutationCreateBoard';
import { useMutationUpdateBoard } from '@/shared/hooks/mutations/useMutationUpdateBoard';
...
import {
  getBoardSchema,
  IFormData,
} from '@/shared/utils/validations/boardSchema';
...
export const useBoardWrite = (props: IBoardWriteDataProps) => {
  ...
  const {
    ...
    resolver: zodResolver(getBoardSchema({ isEdit })) as any,
    mode: 'onChange',
    defaultValues: {
      writer: '',
      password: '',
